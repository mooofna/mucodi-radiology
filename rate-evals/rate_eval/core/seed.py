"""RNG seeding helper for reproducible CV evaluation."""
...
class Seed:
    seed: ClassVar[Optional[int]] = None
    ...
    def get_loader_worker_init(cls) -> Callable[[int], None]:
        if cls._is_set():
            return _seed_worker
        raise RuntimeError("Seed has not been set. Call Seed.set(seed) before using a DataLoader.")
